...
option = float(input(
    'Which design option do you want to analyze? (1 - high stringer low thickness, 2 - low stringer high thickness, 3 - intermediate) '))
...
corr_I_x_s = (h * (
            4 * a * b * c ** 2 + 3 * a ** 2 * b * c - 3 * a * b ** 2 * c + a ** 4 + b ** 4 + 2 * a ** 3 * b + a ** 2 * c ** 2 + a ** 3 * c + 2 * a * b ** 3 - c * b ** 3 + b ** 2 * c ** 2)) / (
                         36 * (a + b))
# x Moment of inertia of the cut-out trapezoid:
corr_I_x_c = (h1 * (
            4 * a1 * b1 * c1 ** 2 + 3 * a1 ** 2 * b1 * c1 - 3 * a1 * b1 ** 2 * c1 + a1 ** 4 + b1 ** 4 + 2 * a1 ** 3 * b1 + a1 ** 2 * c1 ** 2 + a1 ** 3 * c1 + 2 * a1 * b1 ** 3 - c1 * b1 ** 3 + b1 ** 2 * c1 ** 2)) / (
                         36 * (a1 + b1))
...
I_xx_s1 = (L * (t ** 3) / 12 + t * L * (y_bar - t / 2) ** 2 + t * ((L - t) ** 3) / 12 + t * (L - t) * (
            y_bar - (t + (L - t) / 2)) ** 2)
I_xx_s = np.zeros(100)
for i in range(100):
    I_xx_s[i] = I_xx_s1
