...
def calculate_aeroloads(lines10, rho, v, q):
    ...
    def desired_CL(CLd):
        ...
        for i in range(100):
            CLd_y = float(liftcoefficients0[i]) + ((CLd - CL0) / (CL10 - CL0)) * (
                        float(liftcoefficients10[i]) - float(liftcoefficients0[i]))
            CMd_y = float(momentcoefficients0[i]) + ((CLd - CL0) / (CL10 - CL0)) * (
                        float(momentcoefficients10[i]) - float(momentcoefficients0[i]))
            Cidd_y = float(dragcoefficients0[i]) + ((CLd - CL0) / (CL10 - CL0)) * (
                        float(dragcoefficients10[i]) - float(dragcoefficients0[i]))
    ...
    aero_torque_CLd_1, aero_moment_CLd_1, aero_shear_CLd_1, aero_drag_CLd_1 = calculate_lift(CLd_distributed_1,
                                                                                             alpha_d_1,
                                                                                             CMd_distributed_1,
                                                                                             Cidd_distributed_1)
    aero_torque_CLd_25, aero_moment_CLd_25, aero_shear_CLd_25, aero_drag_CLd_25 = calculate_lift(CLd_distributed_25,
                                                                                                 alpha_d_25,
                                                                                                 CMd_distributed_25,
                                                                                                 Cidd_distributed_25)
    aero_torque_CLd_min, aero_moment_CLd_min, aero_shear_CLd_min, aero_drag_CLd_min = calculate_lift(
        CLd_distributed_min, alpha_d_min, CMd_distributed_min, Cidd_distributed_min)
